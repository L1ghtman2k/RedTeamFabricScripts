...
def host(user, net, number_of_teams, first_team_start_address, lan, dmz, lan_list, dmz_list, wan_ip, gateway_user, *args, **kwargs):
    hosts_list = []
    la = [lan, lan_list]
    dm = [dmz, dmz_list]
    for team_number in range(first_team_start_address, first_team_start_address+number_of_teams):
        for zone in [la, dm]:
            if zone[1]:
                single = f"{user}@{net}.{team_number}.{zone[0]}."
                for fourth_octet in zone[1].split(','):
                    hosts_list.append(single+fourth_octet)
        if wan_ip:
